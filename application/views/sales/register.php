...
<div class="clear"></div>
	...
	<div class="row">
		...
		<?php if ($mode == 'return'){	?>
			<input type="hidden" id="tipo_documento_return" name="tipo_documento_return" value="">
			<div class="row">
				<div>
					<ul class="list-inline pull-left">
						<li><label id="correlativo_label" for="correlativo">Documento:&nbsp;</label><?=form_input(array('name'=>'correlativo_return','value'=> $_SESSION['correlativo_return'],'size'=>'10', 'id' => 'correlativo_return'))?></li>
						<li><?=form_radio(array('name'=>'factura_return','id'=>'return_sale_button1','value'=>'1','class'=>'email_receipt_checkbox','checked'=> ''))?><label id='correlativo_label' for='correlativo'>&nbsp;Factura&nbsp;</label></li>
						<li><?=form_radio(array('name'=>'factura_return','id'=>'return_sale_button2','value'=>'1','class'=>'email_receipt_checkbox','checked'=> ''))?><label id='correlativo_label' for='correlativo'>&nbsp;Boleta&nbsp;</label></li>
					 	<li><?=form_radio(array('name'=>'factura_return','id'=>'return_sale_button3','value'=>'1','class'=>'email_receipt_checkbox','checked'=> ''))?><label id='correlativo_label' for='correlativo'>&nbsp;Gu&iacute;a&nbsp;</label></li>
						<li><a href='javascript:void(0);' class='btn btn-success' id='return_sale_button'>Buscar</a></li>
					</ul>
				</div>		
			</div>		
		<?php }	?>	
							...
							foreach(array_reverse($cart, true) as $line=>$item)	
							{
								...
								<tr id="reg_item_top" bgcolor="#eeeeee" >
									...
									<td>
										...
										echo form_input(array('name'=>'price','value'=>to_currency_no_money($item['price'], 0),'class'=>'input-small', 'id' => 'price_'.$line));?>
									...
									<td class="text text-main"><?php echo to_currency($item['price']*$item['quantity']-$item['price']*$item['quantity']*$item['discount']/100, 0); ?></td>
								</tr>
		...
		<ul class="list-inline pull-left">
			...
			<?php } ?>
				...
				<?php } ?>	
					...
					<?php  if((count($payments) > 0 && !is_sale_integrated_cc_processing())){?>
	<br>
	<br>
	<br>
						...
						<ul class="list-inline pull-left">
							...
							$sql="
								...
								FROM  phppos_sales_docs
								WHERE phppos_sales_docs.tipo_documento = 'factura'
								LIMIT 0, 1";
		 					$result=mysql_query($sql);
		 					$numrows=mysql_num_rows($result);
		 					$row=mysql_fetch_assoc($result);
		 					$ultimo_correlativo = $row['ultimo_correlativo'];
		 					echo "<input id='ultima_factura' type='hidden' name='ultima_factura' value='".($ultimo_correlativo+1)."'>";
		 					$sql="
								SELECT
									  MAX(correlativo) AS ultimo_correlativo
								FROM  phppos_sales_docs
								WHERE phppos_sales_docs.tipo_documento = 'boleta'
								LIMIT 0, 1";
		 					$result=mysql_query($sql);
		 					$numrows=mysql_num_rows($result);
		 					$row=mysql_fetch_assoc($result);
		 					$ultimo_correlativo = $row['ultimo_correlativo'];
		 					echo "<input id='ultima_boleta' type='hidden' name='ultima_boleta' value='".($ultimo_correlativo+1)."'>";
		 					$sql="
								SELECT
									  MAX(correlativo) AS ultimo_correlativo
								FROM  phppos_sales_docs
								WHERE phppos_sales_docs.tipo_documento = 'guia'
								LIMIT 0, 1";
		 					$result=mysql_query($sql);
		 					$numrows=mysql_num_rows($result);
		 					$row=mysql_fetch_assoc($result);
		 					$ultimo_correlativo = $row['ultimo_correlativo'];
		 					echo "<input id='ultima_guia' type='hidden' name='ultima_guia' value='".($ultimo_correlativo+1)."'>";
		 					
							if ($payments_cover_total && $customer_required_check)
							{
								echo '<li><label id="correlativo_label" for="correlativo">Correlativo:&nbsp;</label>'.form_input(array('name'=>'correlativo','value'=> '','size'=>'10', 'id' => 'correlativo')).'</li>';
								if ($customer_id > 0){
									/*echo "<li><<a href='javascript:void(0);' class='btn btn-success' id='finish_sale_button'>Factura</a></li>";*/
									echo "<li>".form_radio(array('name'=>'factura','id'=>'finish_sale_button','value'=>'1','class'=>'email_receipt_checkbox','checked'=> ''))."<label id='correlativo_label' for='correlativo'>&nbsp;Factura&nbsp;</label></li>";
								}
								/*echo "<li><a href='javascript:void(0);' class='btn btn-success' id='finish_sale_button2'>Boleta</a></li>";*/
								echo "<li>".form_radio(array('name'=>'factura','id'=>'finish_sale_button2','value'=>'1','class'=>'email_receipt_checkbox','checked'=> ''))."<label id='correlativo_label' for='correlativo'>&nbsp;Boleta&nbsp;</label></li>";
								if ($customer_id > 0){
									/*echo "<li><a href='javascript:void(0);' class='btn btn-success' id='finish_sale_button3'>Gu&iacute;a</a></li>";*/ 	
									echo "<li>".form_radio(array('name'=>'factura','id'=>'finish_sale_button3','value'=>'1','class'=>'email_receipt_checkbox','checked'=> ''))."<label id='correlativo_label' for='correlativo'>&nbsp;Gu&iacute;a&nbsp;</label></li>";
								}
								echo "<li><a href='javascript:void(0);' class='btn btn-success' id='finish_sale_button4'>Emitir Documento</a></li>";
								/*
								echo "<li>".anchor(site_url('sales/closeregister?continue=home'), 'Boleta', array('class'=>'btn btn-success', 'id'=>'finish_sale_button'))."</li>";
								echo "<li>".anchor(site_url('sales/closeregister?continue=home'), 'Gu&iacute;a', array('class'=>'btn btn-success', 'id'=>'finish_sale_button'))."</li>";
								
								echo "<li><div class='small_button'><input type='button' class='btn btn-success btn-large btn-block' id='finish_sale_button' value='Factura' /></div></li>";
								echo "<li><div class='small_button'><input type='button' class='btn btn-success btn-large btn-block' id='finish_sale_button' value='Boleta' /></div></li>";
								echo "<li><div class='small_button'><input type='button' class='btn btn-success btn-large btn-block' id='finish_sale_button' value='Gu&iacute;a' /></div></li>";
								*/
							}
							?>
							</ul>
									...
									<ul class="list-unstyled">
										<li id="customer_view_modal"><strong><a href="<?php echo isset($customer_id) ? site_url('customers/view_modal/'.$customer_id) : site_url('customers/view_modal/'.$customer_id) ; ?>" data-toggle="modal" data-target="#myModal" ><?php echo character_limiter(H($customer), 25); ?></a></strong>
											<?php if ($this->config->item('customers_store_accounts') && isset($customer_balance)) {?>
												<span class="<?php echo $is_over_credit_limit ? 'credit_limit_warning' : 'credit_limit_ok'; ?>">(<?php echo lang('customers_balance').': '.to_currency($customer_balance, 0); ?>)</span>
										<?php } ?>
										</li>
					...
					<div id="add_customer_info">
							<div id="common_or" class="common_or">
								...
								echo anchor("customers/view/-1/1",
									"<div class='small_button'> <span>Nuevo Cliente</span> </div>", array('class'=>'btn btn-primary none','title'=>lang('sales_new_customer'), 'id' => 'new-customer'));
								?>
							</div>
					</div>
*/?>
					...
					<div id='sale_details'>
						<table id="sales_items" class="table">
							...
							<tr class="error">
								...
								<td class="right"><?php echo to_currency($this->Giftcard->get_giftcard_value(end($giftcard_payment_row)) - $payment['payment_amount'], 0);?></td>
							</tr>
							<?php }?>
							<?php }?>
							<tr class="info">
								<td class="left">Neto :</td>
								<td class="right"><?php echo to_currency($subtotal, 0); ?></td>
							</tr>
							...
							<tr class="color1">
								<td class="left">
									...
									<?php } ?>
									<?php echo $name; ?>:</td>
								<td class="right"><?php echo to_currency($value, 0); ?></td>
							</tr>
							<?php }; ?>
							<tr class="success">
								<td ><h3 class="sales_totals">Total :</h3></td>
								<td ><h3 class="currency_totals"><?php echo to_currency($total, 0); ?></h3></td>
							</tr>
							<?php if($total%5!=0){	?>
								<?php /*redirect('sales/reload');*/?>
								<tr class="success">
									<form id="ajuste" action="<?=site_url('sales/ajuste_sencillo')?>" method="post">
										<input type="hidden" name="neto" value="<?=$subtotal?>">
								
										<input type="hidden" name="total" value="<?=$total?>">
									</form>
									<td colspan="2"><input type='button' class='btn btn-success btn-large btn-block' id='ajuste_sencillo' value='Ajuste de Sencillo' /></td>
								</tr>
							<?php }	?>
						</table>
					</div>
						...
						?>
								...
								<tbody id="payment_contents">
									...
									foreach($payments as $payment_id=>$payment)
									{
										...
										<tr class="warning">
											...
											<td id="pt_amount"><?php echo  to_currency($payment['payment_amount'], 0); ?>  </td>
										</tr>
										<?php
									}
									?>
							...
							<table id="amount_due" class="table">
								<tr class="<?php if($payments_cover_total) { echo 'success'; } else { echo 'error'; }?>">
									<td>
										<h4 class="sales_amount_due">Total a Pagar :</h4>
									</td>
									<td>
										<h3 class="amount_dues"><?php echo to_currency($amount_due, 0); ?></h3>
									</td>
								</tr>
							</table>
								...
								<table id="make_payment_table" class="table">
									...
									<tr id="mpt_bottom" >
										<td id="tender" colspan="2">
											<div class="input-append">
												<?php echo form_input(array('name'=>'amount_tendered','id'=>'amount_tendered','value'=>to_currency_no_money($amount_due, 0),'class'=>'input-medium input_mediums', 'accesskey' => 'p'));	?>
				...
				<li class="list-group-item">
					...
					echo 'Incluir comentarios en el documento';
					echo form_checkbox(array(
				...
				if((count($payments) > 0 && !is_sale_integrated_cc_processing())){?>
					...
					<div id="finish_sale">
						<?php echo form_open("sales/complete",array('id'=>'finish_sale_form', 'autocomplete'=> 'off')); ?>
							...
							if ($payments_cover_total && $customer_required_check)
							{
								echo "<input type='button' class='btn btn-success btn-large btn-block' id='finish_sale_button' value='Factura' />";
								echo "<input type='button' class='btn btn-success btn-large btn-block' id='finish_sale_button' value='Boleta' />";
								echo "<input type='button' class='btn btn-success btn-large btn-block' id='finish_sale_button' value='Gu&iacute;a' />";
							}
							?>
						</form>
					</div>
...
$(document).keydown(function(event)
{
	...
	$(document).ready(function()
	{
		...
		$( "#ajuste_sencillo" ).click(function(e) {
		...
		$("#finish_sale_button").click(function()
		{
			...
			$("#correlativo").val($("#ultima_factura").val());
		...
		//finish_sale_button2
		$("#finish_sale_button2").click(function()
		{
			//Prevent double submission of form
			
			$("#tipo_documento").val('boleta');
			$("#correlativo").val($("#ultima_boleta").val());  
		...
		//finish_sale_button3
		$("#finish_sale_button3").click(function()
		{
			
			$("#tipo_documento").val('guia');
			$("#correlativo").val($("#ultima_guia").val()); 
		...
		$("#finish_sale_button4").click(function()
		{
			//Prevent double submission of form
			$("#finish_sale_button4").hide();
		...
		//return_sale_button
		$("#return_sale_button1").click(function()
		{
		
			$("#tipo_documento_return").val('factura');
		...
		//return_sale_button2
		$("#return_sale_button2").click(function()
		{
			//Prevent double submission of form
			
			$("#tipo_documento_return").val('boleta');
		...
		//return_sale_button3
		$("#return_sale_button3").click(function()
		{
			
			$("#tipo_documento_return").val('guia');
			
			
		});
		//return_sale_button3
		
		$("#return_sale_button").click(function()
		{
			//Prevent double submission of form
		
			$("#register_container").mask(<?php echo json_encode(lang('common_wait')); ?>);
			var correlativo_return = $("#correlativo_return").val();
			var tipo_documento_return = $("#tipo_documento_return").val();

			var url_edit = '<?php echo site_url("sales/change_sale2");?>/'+correlativo_return+'/'+tipo_documento_return;
