	...
	*/
	...
	function view($item_id=-1,$redirect=0, $sale_or_receiving = 'sale')
	{
      ...
      $this->load->helper('report');
		...
		$data['opt_items_tipo'] = $this->Item->opt_items_tipo();
	...
	function save($item_id=-1)
	{
		...
		'category'=>$this->input->post('category'),
		'id_items_tipo'=>$this->input->post('id_items_tipo'),
		...
		{	
			if (($handle = fopen($_FILES['file_path']['tmp_name'], "r")) !== FALSE)
			{
				...
				for($k = 2;$k<=$num_rows; $k++)
				{
					...
					$item_id = $sheet->getCellByColumnAndRow(15+$price_tiers_count, $k)->getValue();
