...
class Customers extends Person_controller
{
	...
	*/
	function view($customer_id=-1,$redirect_code=0)
	{
		...
		$data['person_info']=$this->Customer->get_info_alternativa($customer_id);
		 
		$data['url_combo_ciudad']   = site_url('admin_localidad/combo_ciudad');
		$data['tipo_persona']		= array(''=>'Seleccione') + $this->tipo_persona;	
		$data['opt_lista_precio']  	= array(0=>'Sin Lista de Precio') + $this->Lista_precio->opt_lista_precio();
		$data['opt_giro']  			= array(0=>'Persona Natural') + $this->Giro->opt_giros();
		$data['opt_region']         = $this->Localidad->opt_region();
		$data['opt_ciudad']         = $this->Localidad->opt_ciudad_por_region($data['person_info']->id_region);
