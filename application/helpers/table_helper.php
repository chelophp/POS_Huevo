...
function get_people_manage_table($people,$controller)
{
	...
	//echo "ECHO : " .  $CI->config->item('customers_store_accounts');
...
function get_person_data_row($person,$controller)
{
	...
	$table_data_row.='<td ><a href="'.$link.'" class="underline">'.H($person->rut).'</a></td>';
	if ($person->tipo_persona == 'natural' || $person->tipo_persona == 'extranjero'){
		$nombre_cliente = $person->first_name.' '.$person->last_name;
	}else{//juridica
		$nombre_cliente = $person->first_name;
	}
	
	$table_data_row.='<td><a href="'.$link.'" class="underline">'.H($nombre_cliente).'</a></td>';
...
function get_item_data_row($item,$controller)
{
	...
	$table_data_row.='<td width="9%">'.H($item->nombre_items_tipo).'</td>';
...
function get_location_data_row($location,$controller)
{
	...
	$table_data_row.='<td width="11%">'.$location->utilidad.'</td>';
