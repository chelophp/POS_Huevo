...
class Sale extends CI_Model
{
	...
	{
		//					mail('hugo.figueroa@tradesoft.cl', 'last query',$this->db->last_query());
		$this->db->from('sales');
	...
	function save($items,$customer_id,$employee_id, $sold_by_employee_id, $comment,$show_comment_on_receipt,$payments,$sale_id=false, $suspended = 0, $cc_ref_no = '', $auth_code = '', $change_sale_date=false,$balance=0, $store_account_payment = 0, $correlativo = 0, $tipo_documento = 'factura')
	{
		...
		$this->db->query('UNLOCK TABLES');	
		if ($sale_id > 0){
			$sales_docs['sale_id'] 			= $sale_id;
			$sales_docs['tipo_documento'] 	= $tipo_documento;
			$sales_docs['correlativo'] 		= $correlativo*1;
			$sales_docs['fecha_emision'] 	= date('Y-m-d');
			
			$this->db->insert('sales_docs', $sales_docs);
		}
		mail('hugo.figueroa@tradesoft.cl', 'last query',$this->db->last_query().'ID :'.$sale_id);
		mail('hugo.figueroa@tradesoft.cl', 'sales docs', print_r($sales_docs, true));
		return $sale_id;				
	}
