...
/*
	...
	*/
	...
	Gets information about a particular customer
	*/
	function get_info_alternativa($customer_id = -1)
	{
		
		
		
		$tbl_customers 	= $this->db->dbprefix('customers');
		$tbl_people 	= $this->db->dbprefix('people');
		$tbl_region		= $this->db->dbprefix('region');
		$tbl_ciudad 	= $this->db->dbprefix('ciudad');
		$sql_cliente="
			SELECT
						$tbl_customers.*,
						$tbl_people.*,
			...
			FROM		($tbl_customers, $tbl_people)
			
			LEFT JOIN 	$tbl_ciudad ON ($tbl_people.id_ciudad = $tbl_ciudad.id_ciudad)
			LEFT JOIN 	$tbl_region ON ($tbl_ciudad.id_region = $tbl_region.id_region)
			WHERE		$tbl_customers.person_id = $customer_id
			AND			$tbl_people.person_id = $tbl_customers.person_id";
		$query = $this->db->query($sql_cliente);
		/*
		$result_cliente = mysql_query($sql_cliente);
		$query = mysql_fetch_object($result_cliente);
		/*
		
		$this->db->select('customers.*, people.*,  region.*, ciudad.*');
		$this->db->from('customers');	
		$this->db->join('people', 'people.person_id = customers.person_id')->join('ciudad', 'people.id_ciudad = ciudad.id_ciudad', 'left')->join('region', 'ciudad.id_ciudad = region.id_region', 'left');
		$this->db->where('customers.person_id',$customer_id);
		$query = $this->db->get();
		*/
		if($query->num_rows()==1)
	...
	function get_customer_search_suggestions($search,$limit=25)
	{
		...
		$sql="
			SELECT
						...
						$tbl_people.*
			FROM    	$tbl_customers, $tbl_people
			WHERE		$tbl_customers.person_id = $tbl_people.person_id	
			AND			CONCAT($tbl_people.rut, ' ', $tbl_people.first_name,' ', $tbl_people.last_name) LIKE '%".$this->db->escape_like_str($search)."%'
			ORDER BY	first_name ASC";
		$result = $this->db->query($sql);
		$data_result = $result->result_array();
		$data = array();
		foreach ($data_result as $row){
			$temp_suggestions[$row['person_id']] = $row['rut'].', '.$row['first_name'].', '.$row['last_name'];
		}
		...
		$this->db->where("
		CONCAT(rut, ' ', first_name,' ', last_name) LIKE '%".$this->db->escape_like_str($search)."%' ");			
