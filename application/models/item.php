...
class Item extends CI_Model
{
	...
	*/
	function get_all($limit=10000, $offset=0,$col='item_id',$order='desc')
	{
		...
		$this->db->join('items_tipo', 'items.id_items_tipo = items_tipo.id_items_tipo');
	...
	*/
	...
	function opt_items_tipo()
	{
		
		$tbl_items_tipo = $this->db->dbprefix('items_tipo');
		$sql="
		SELECT
					$tbl_items_tipo.id_items_tipo,
					$tbl_items_tipo.nombre_items_tipo
		FROM        $tbl_items_tipo
		WHERE       1
		ORDER BY    $tbl_items_tipo.nombre_items_tipo ASC";
	
		$result = $this->db->query($sql);
		$data_result = $result->result_array();
		$data[''] = 'Seleccione';
		foreach ($data_result as $row){
			$data[$row['id_items_tipo']] = $row['nombre_items_tipo'];
		}
