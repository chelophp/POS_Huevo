...
class Sales extends Secure_area
{
	...
	function complete()
	{
		...
		$sale_id_raw = $this->Sale->save($data['cart'], $customer_id, $employee_id, $sold_by_employee_id, $data['comment'],$data['show_comment_on_receipt'],$data['payments'], $suspended_change_sale_id, 0,$data['ref_no'],$data['auth_code'], $data['change_sale_date'], $data['balance'], $data['store_account_payment'], $correlativo, $tipo_documento);
		...
		$sql="
				...
				WHERE	phppos_sales_docs.sale_id = '{$data['sale_id']}'";
		/*echo $sql;*/
		mail('hugo.figueroa@tradesoft.cl', 'test', $sql);
		$resultado=mysql_query($sql);
		$numrows=mysql_num_rows($resultado);
		$rows=mysql_fetch_assoc($resultado);

		$data['tipo_documento'] = $rows['tipo_documento'];
	...
	function receipt($sale_id)
	{
		...
		phppos_sales_docs.*
		FROM	phppos_sales_docs
		WHERE	phppos_sales_docs.sale_id = '{$sale_id}'";
		/*echo $sql;*/
		mail('hugo.figueroa@tradesoft.cl', 'test', $sql);
		$resultado=mysql_query($sql);
				$numrows=mysql_num_rows($resultado);
	...
	function ajuste_sencillo(){
		...
		while ($total%5!=0){
			...
			$quantity = 1;
			$this->sale_lib->add_item($item_id_or_number_or_item_kit_or_receipt,$quantity);
			$total	=	$this->sale_lib->get_total();
		}
		redirect('sales');
	}
	...
	function retorno_venta($correlativo = 0, $tipo_documento = '')
	{
		$sale_id = $correlativo;
		$data['cart']=$this->sale_lib->get_cart();
		$data['subtotal']=$this->sale_lib->get_subtotal();
	...
	function change_sale2($sale_id, $tipo_documento)
	{
		$_SESSION['mode'] = 'return';
		$_SESSION['correlativo_return'] = $sale_id;
		$this->check_action_permission('edit_sale');
		$this->sale_lib->clear_all();
		$this->sale_lib->copy_entire_sale($sale_id);
		$this->sale_lib->set_change_sale_id($sale_id);
