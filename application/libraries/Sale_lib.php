...
class Sale_lib
{
	...
	function get_price_for_item($item_id, $tier_id = FALSE)
	{
		...
		$row_location 		= $this->CI->Location->get_info($location_id);
		$row_customer 		= $this->CI->Customer->get_info($_SESSION['customer']);
		$row_lista_precio 	= $this->CI->Lista_precio->get_info($row_customer->id_lista_precio);
		...
		$item_location_tier_row = $this->CI->Item_location->get_tier_price_row($tier_id, $item_id, $location_id);
		...
		if (!empty($item_location_tier_row) && $item_location_tier_row->unit_price){
			...
			if ($is_item_location_promo){
				/*si tiene precio promoción en local(anular precio)*/
				return to_currency_no_money($item_location_info->promo_price, 0);
			}elseif ($is_item_promo){
				/*si tiene precio promoción glogal*/
				return to_currency_no_money($item_info->promo_price, 0);
			}else{
				/*si tiene precio definido en local(anular precio)*/
				if ($item_location_info->unit_price){
					$item_unit_price = $item_location_info->unit_price;
					$item_unit_price = $item_location_info->cost_price + (($item_location_info->cost_price * $row_location->utilidad) / 100);
					if ($row_customer->id_lista_precio > 0 && $_SESSION['customer'] > 0){
						$item_unit_price = $item_unit_price - (($item_unit_price * $row_lista_precio->descuento) / 100);
					}
				}else{
					$item_unit_price = $item_info->unit_price;
					$item_unit_price = $item_info->cost_price + (($item_info->cost_price * $row_location->utilidad) / 100);
					if ($row_customer->id_lista_precio > 0 && $_SESSION['customer'] > 0){
						$item_unit_price = $item_unit_price - (($item_unit_price * $row_lista_precio->descuento) / 100);
					}
				}
				return to_currency_no_money($item_unit_price, 0);
			}
		}			
	...
	function get_price_for_item_kit($item_kit_id, $tier_id = FALSE)
	{
		...
		{
			$item_kit_unit_price = $item_kit_location_info->unit_price ? $item_kit_location_info->unit_price : $item_kit_info->unit_price;
			if ($item_kit_location_info->unit_price){
				$item_kit_unit_price = $item_kit_location_info->unit_price;
				$item_kit_unit_price = $item_kit_location_info->cost_price + (($item_kit_location_info->cost_price * $row_location->utilidad) / 100);
				$item_kit_unit_price = $item_kit_unit_price - (($item_kit_unit_price * $row_lista_precio->descuento) / 100);
			}else{
				$item_kit_unit_price = $item_kit_info->unit_price;
				$item_kit_unit_price = $item_kit_info->cost_price + (($item_kit_info->cost_price * $row_location->utilidad) / 100);
				$item_kit_unit_price = $item_kit_unit_price - (($item_kit_unit_price * $row_lista_precio->descuento) / 100);
			}
			return to_currency_no_money($item_kit_unit_price, 0);
		}		
	}	
	...
	*/
	function change_price()
	{
		...
		foreach ($items as $item )
		{
			if (isset($item['item_id']))
			{
				...
				$previous_price = to_currency_no_money($previous_price, 0);
				$price = to_currency_no_money($price, 0);
	...
	function add_item2($item_id,$quantity=1,$discount=0,$price=null,$description=null,$serialnumber=null, $force_add = FALSE, $line = FALSE)
	{
	...
	function edit_item($line,$description = FALSE,$serialnumber = FALSE,$quantity = FALSE,$discount = FALSE,$price = FALSE)
	{
		...
		$item_info = $this->CI->Item->get_info($item_id);
		$price_to_use = $item_info->unit_price;
		$price = $price_to_use;
		*/
		...
		$sql="
			SELECT
						phppos_items_precio_por_cantidad.precio_unitario	
			FROM		phppos_items_precio_por_cantidad
			WHERE		phppos_items_precio_por_cantidad.item_id = '$item_id'
			AND			phppos_items_precio_por_cantidad.cantidad <= $quantity
			ORDER BY 	phppos_items_precio_por_cantidad.cantidad DESC
			LIMIT 		0, 1";
		$result=mysql_query($sql);
		$numrows=mysql_num_rows($result);
		$rows=mysql_fetch_assoc($result);
		mail('hugo.figueroa@tradesoft.cl', 'datos', '-item_id:'.$item_id.'-precio original:'.$price_to_use.'-line:'.$line.'-descripcion:'.$description.'-serial:'.$serialnumber.'-cantidad:'.$quantity.'-desceunto:'.$discount.'-precio:'.$price.'-precio_nuevo'.$rows['precio_unitario']);
		if ($rows['precio_unitario'] > 0){
			$price = $rows['precio_unitario'];
		}else{
	...
	function recalcular_venta(){
		$location_id 		= $this->CI->Employee->get_logged_in_employee_current_location_id();
		$row_location 		= $this->CI->Location->get_info($location_id);
		$row_customer 		= $this->CI->Customer->get_info($_SESSION['customer']);
		$row_lista_precio 	= $this->CI->Lista_precio->get_info($row_customer->id_lista_precio);
		/*
		$row_location->utilidad;
		*/
		
		/*logica de precios sin cliente*/
		
		if ($row_customer->id_lista_precio == 0 || empty($row_customer->id_lista_precio) || $_SESSION['customer'] == -1){
			
			
			$items_recalcular = $this->get_cart();
			foreach ($items_recalcular as $line => $items)
			{
				$item_id 		= 0;
				$item_kit_id 	= 0;
				$item_id 		= $this->get_item_id($line);
				$item_kit_id 	= $this->get_kit_id($line);
				if ($item_id > 0){
					$item_info 			= $this->CI->Item->get_info($item_id);
					$arr_item_line[$line]  	= $item_id;
					$arr_items_id[$item_id] += $items['quantity'];
				}
				if ($item_kit_id > 0){
					for($k=0;$k<$items['quantity'];$k++){
						foreach ($this->CI->Item_kit_items->get_info($item_kit_id) as $item_kit_item)
						{
							$arr_item_line[$line]	= $item_kit_item->item_id;
							$arr_items_id[$item_kit_item->item_id] += $item_kit_item->quantity;
						}
					}
				}
			}
			
			foreach ($arr_items_id as $item_id => $cantidad){
				$sql="
					SELECT
								phppos_items.id_items_tipo
					FROM		phppos_items
					WHERE		phppos_items.item_id = '$item_id'";
				$result=mysql_query($sql);
				$numrows=mysql_num_rows($result);
				$rows=mysql_fetch_assoc($result);
				$arr_tipo_cantidad[$rows['id_items_tipo']] += $cantidad;
				$arr_item_id_tipo[$item_id] = $rows['id_items_tipo'];
			}
			
			foreach ($arr_tipo_cantidad as $id_items_tipo => $cantidad){
				$sql="
					SELECT
								phppos_items_descuento_por_tipo.id_items_descuento_por_tipo,
								phppos_items_descuento_por_tipo.id_items_tipo,
								phppos_items_descuento_por_tipo.cantidad,
								phppos_items_descuento_por_tipo.descuento
					FROM		phppos_items_descuento_por_tipo
					WHERE		phppos_items_descuento_por_tipo.id_items_tipo = '$id_items_tipo'
					AND			phppos_items_descuento_por_tipo.cantidad <= $cantidad
					ORDER BY 	phppos_items_descuento_por_tipo.cantidad DESC
					LIMIT 		0, 1";
				$result=mysql_query($sql);
				$numrows=mysql_num_rows($result);
				$rows=mysql_fetch_assoc($result);
				$arr_tipo_descuento[$id_items_tipo] = $rows['descuento'];
			}
			...
			foreach ($items_recalcular as $line => $items)
			{
				...
				if(isset($value[$line]))
				{
					$discount = $arr_tipo_descuento[$arr_item_id_tipo[$arr_item_line[$line]]];
					if ($discount !== FALSE ) {
						$value[$line]['discount'] = $discount;
					}
					if (empty($discount)){
		...
		/*logica de precios por lista de precio*/
		if ($row_customer->id_lista_precio > 0 && $_SESSION['customer'] > 0){
			/*
			$row_lista_precio->descuento;
			$items_recalcular = $this->get_cart();
			foreach ($items_recalcular as $line => $items)
			{
				$value[$line] = $items;
				...
				{
					if ($discount !== FALSE ) {
						$value[$line]['price'] = round($value[$line]['price'] + ($value[$line]['price'] * $row_location->utilidad / 100));
					}
					$this->set_cart($value);
				}
			}
		...
		*/
		...
		while ($total%5!=0){
			$item_id_or_number_or_item_kit_or_receipt = 10;
			$quantity = 1;
			$this->add_item2($item_id_or_number_or_item_kit_or_receipt,$quantity);
			$total	=	$this->get_total();
		}
	...
	function get_subtotal($sale_id = FALSE)
	{
		...
		foreach($this->get_cart() as $item)
		{
		    ...
		    $subtotal+=round($price_to_use*$item['quantity']-$price_to_use*$item['quantity']*$item['discount']/100);
		}
		
		return to_currency_no_money($subtotal, 0);
	}
